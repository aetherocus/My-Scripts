...
local RemoteName = Main:CreateInput({
   ...
   local Keybind = Main:CreateKeybind({
             ...
             Flag = "keybinded", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
...
local libtp = Tps:CreateButton({
	...
	Callback = function()
        char.PrimaryPart.CFrame = CFrame.new(1933, 5, -1463)
	end,
})
